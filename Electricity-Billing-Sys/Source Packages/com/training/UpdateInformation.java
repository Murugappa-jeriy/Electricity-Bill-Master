...
public class UpdateInformation extends JFrame implements ActionListener{
    ...
    UpdateInformation(String meter){
        ...
        try{
            ...
            ResultSet rs = c.s.executeQuery("select * from customer_table where meter = '"+meter+"'");
            while(rs.next()){
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            ...
            try{
            	Connectivity c = new Connectivity();
                c.s.executeUpdate("update customer_table set address = '"+s3+"', city = '"+s4+"', state = '"+s5+"', email = '"+s6+"', phone_no = '"+s7+"' where meter = '"+meter+"'");
